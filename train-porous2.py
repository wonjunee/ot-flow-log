...
parser.add_argument(
    '--data', choices=['square', 'porous', 'porous2', 'swissroll', '8gaussians','2gaussians', '1gaussians', 'pinwheel', 'circles', 'moons', '2spirals', 'checkerboard', 'rings'],
...
parser.add_argument('--lr_drop'     , type=float, default=1.0001)
...
def compute_loss_gf(net, x, nt, n_tau, net_list, rho, z):
    Jc , cs = OTFlowPorous(x, rho, net, [0,1], nt=nt, n_tau=n_tau, net_list=net_list, stepper="rk4", alph=net.alph, z=z)
    return Jc, cs
...
if __name__ == '__main__':
    ...
    for itr in range(0, args.niters + 1):
        ...
        optim.zero_grad()
        loss, costs  = compute_loss_gf(net, x0, nt=nt, n_tau=n_tau, net_list=net_list, rho=rho_next, z=z)
        loss.backward()
        optim.step()
        ...
        if loss < global_loss:
            global_loss = loss
            best_params = net.state_dict()
        # elif loss > global_loss * 1.1: # if the loss increases then it means lr is too high -> decrease it
        ...
        if itr % args.viz_freq == 0:
            ...
            with torch.no_grad():
                ...
                # ax.plot(x_next[:,0], rho_next, 'o', alpha=0.5)
                # ax.set_xlabel("$x$")
                # ax.set_ylabel("$\\rho$")
                # ax.set_xlim([-3,3])
                # ax.set_ylim([0,1.3])
                # plt.savefig(os.path.join(args.save, f'plot_n_tau_{n_tau}.png'))
                # sys.exit()
